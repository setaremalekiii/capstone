...
def load_images_by_class(cutouts_dir: str):
    base = Path(cutouts_dir)
    grouped = defaultdict(list)

    for cls_dir in sorted(base.glob("class_*")):
        if not cls_dir.is_dir():
            continue

        try:
            cls = int(cls_dir.name.split("_")[1])
        except:
            continue

        items = []
        for img_path in sorted(cls_dir.glob("det_*_conf_*.png")):
            mask_path = img_path.with_name(
                img_path.name.replace("_conf_", "_mask_").split(".png")[0] + ".png"
            )
            meta_path = img_path.with_name(
                img_path.name.split("_conf_")[0] + "_meta.json"
            )
...
def main():
    ...
    if not grouped:
        raise RuntimeError("No SAM cutouts found.")

    karyo, layout = build_karyogram(grouped)

    cv2.imwrite(OUT_PATH, karyo)
