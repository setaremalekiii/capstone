...
# 4) Tabs:
#    - Karyogram:            karyogram.png (found in sam_cutouts root or karyo script dir)
#    - Straight Karyogram:   /Users/saatvik_11/Desktop/KaryogramUI_2/Karyogram_straight.png
#    - P-arm Karyogram:      karyogram_parm.png (in sam_cutouts root, if present)
#
...
class EditorWindow(QMainWindow):
    def __init__(self, root_dir: str, karyo_script: str | None):
        super().__init__()
        self.setWindowTitle("SAM Mask Editor + Karyograms")

        self.root_dir = root_dir
        self.karyo_script = karyo_script

        # HARD-CODED straight karyogram path (as requested)
        self.straight_karyogram_abs_path = "/Users/saatvik_11/Desktop/KaryogramUI_2/Karyogram_straight.png"
        self.parm_karyogram_abs_path = "/Users/saatvik_11/Desktop/KaryogramUI_2/Karyogram_parm.png"
        ...
        # -------------------------
        # Straight karyogram tab (ABS path)
        # -------------------------
        self.karyo_straight_label = QLabel("Karyogram_straight.png not found.")
        self.karyo_straight_label.setAlignment(Qt.AlignmentFlag.AlignCenter)

        self.karyo_straight_zoom_slider = QSlider(Qt.Orientation.Horizontal)
        self.karyo_straight_zoom_slider.setMinimum(10)
        self.karyo_straight_zoom_slider.setMaximum(300)
        self.karyo_straight_zoom_slider.setValue(100)
        self.karyo_straight_zoom_slider.valueChanged.connect(self.refresh_straight_view)

        self.karyo_straight_status = QLabel("")
        self.karyo_straight_status.setTextInteractionFlags(Qt.TextInteractionFlag.TextSelectableByMouse)

        kscroll_straight = QScrollArea()
        kscroll_straight.setWidgetResizable(True)
        kscroll_straight.setAlignment(Qt.AlignmentFlag.AlignCenter)
        kscroll_straight.setWidget(self.karyo_straight_label)

        kary_straight_controls = QHBoxLayout()
        kary_straight_controls.setContentsMargins(10, 10, 10, 6)
        kary_straight_controls.addWidget(QLabel("Zoom"))
        kary_straight_controls.addWidget(self.karyo_straight_zoom_slider)
        kary_straight_controls.addStretch(1)
        kary_straight_controls.addWidget(self.karyo_straight_status)

        kary_straight_layout = QVBoxLayout()
        kary_straight_layout.setContentsMargins(0, 0, 0, 0)
        kary_straight_layout.setSpacing(0)
        kary_straight_layout.addLayout(kary_straight_controls)
        kary_straight_layout.addWidget(kscroll_straight)

        kary_straight_tab = QWidget()
        kary_straight_tab.setLayout(kary_straight_layout)

        # -------------------------
        # P-arm karyogram tab (karyogram_parm.png in sam_cutouts root)
        # -------------------------
        self.karyo_parm_label = QLabel("karyogram_parm.png not found.")
        self.karyo_parm_label.setAlignment(Qt.AlignmentFlag.AlignCenter)

        self.karyo_parm_zoom_slider = QSlider(Qt.Orientation.Horizontal)
        self.karyo_parm_zoom_slider.setMinimum(10)
        self.karyo_parm_zoom_slider.setMaximum(300)
        self.karyo_parm_zoom_slider.setValue(100)
        self.karyo_parm_zoom_slider.valueChanged.connect(self.refresh_parm_view)

        self.karyo_parm_status = QLabel("")
        self.karyo_parm_status.setTextInteractionFlags(Qt.TextInteractionFlag.TextSelectableByMouse)

        kscroll_parm = QScrollArea()
        kscroll_parm.setWidgetResizable(True)
        kscroll_parm.setAlignment(Qt.AlignmentFlag.AlignCenter)
        kscroll_parm.setWidget(self.karyo_parm_label)

        kary_parm_controls = QHBoxLayout()
        kary_parm_controls.setContentsMargins(10, 10, 10, 6)
        kary_parm_controls.addWidget(QLabel("Zoom"))
        kary_parm_controls.addWidget(self.karyo_parm_zoom_slider)
        kary_parm_controls.addStretch(1)
        kary_parm_controls.addWidget(self.karyo_parm_status)

        kary_parm_layout = QVBoxLayout()
        kary_parm_layout.setContentsMargins(0, 0, 0, 0)
        kary_parm_layout.setSpacing(0)
        kary_parm_layout.addLayout(kary_parm_controls)
        kary_parm_layout.addWidget(kscroll_parm)

        kary_parm_tab = QWidget()
        kary_parm_tab.setLayout(kary_parm_layout)

        # ---- Tabs ----
        self.tabs = QTabWidget()
        self.tabs.addTab(editor_tab, "Mask Editor")
        self.tabs.addTab(kary_tab, "Karyogram")
        self.tabs.addTab(kary_straight_tab, "Straight Karyogram")
        self.tabs.addTab(kary_parm_tab, "P-arm Karyogram")
        ...
        self.karyogram_straight_path = None
        self.karyogram_straight_pix = None
        self.karyogram_parm_path = None
        self.karyogram_parm_pix = None

        self.load_index(0)

        # Load any existing karyograms on disk immediately
        self.load_additional_karyograms()
    ...
    # ---- straight + p-arm loaders / refreshers ----
    def load_additional_karyograms(self):
        # Straight is ABSOLUTE path
        straight_path = self.straight_karyogram_abs_path

        # P-arm is expected in sam_cutouts root
        parm_path = self.parm_karyogram_abs_path

        if os.path.isfile(straight_path):
            pix = QPixmap(straight_path)
            if not pix.isNull():
                self.karyogram_straight_path = straight_path
                self.karyogram_straight_pix = pix
                self.refresh_straight_view()
        else:
            self.karyogram_straight_path = None
            self.karyogram_straight_pix = None
            self.karyo_straight_label.setText(f"Not found:\n{straight_path}")
            self.karyo_straight_status.setText("")

        if os.path.isfile(parm_path):
            pix = QPixmap(parm_path)
            if not pix.isNull():
                self.karyogram_parm_path = parm_path
                self.karyogram_parm_pix = pix
                self.refresh_parm_view()
        else:
            self.karyogram_parm_path = None
            self.karyogram_parm_pix = None
            self.karyo_parm_label.setText(f"Not found:\n{parm_path}")
            self.karyo_parm_status.setText("")

    def refresh_straight_view(self):
        if self.karyogram_straight_pix is None:
            return
        z = self.karyo_straight_zoom_slider.value() / 100.0
        w = max(1, int(self.karyogram_straight_pix.width() * z))
        h = max(1, int(self.karyogram_straight_pix.height() * z))
        scaled = self.karyogram_straight_pix.scaled(
            w, h,
            Qt.AspectRatioMode.KeepAspectRatio,
            Qt.TransformationMode.SmoothTransformation
        )
        self.karyo_straight_label.setPixmap(scaled)
        if self.karyogram_straight_path:
            self.karyo_straight_status.setText(f"{os.path.basename(self.karyogram_straight_path)}   ({w}×{h})")

    def refresh_parm_view(self):
        if self.karyogram_parm_pix is None:
            return
        z = self.karyo_parm_zoom_slider.value() / 100.0
        w = max(1, int(self.karyogram_parm_pix.width() * z))
        h = max(1, int(self.karyogram_parm_pix.height() * z))
        scaled = self.karyogram_parm_pix.scaled(
            w, h,
            Qt.AspectRatioMode.KeepAspectRatio,
            Qt.TransformationMode.SmoothTransformation
        )
        self.karyo_parm_label.setPixmap(scaled)
        if self.karyogram_parm_path:
            self.karyo_parm_status.setText(f"{os.path.basename(self.karyogram_parm_path)}   ({w}×{h})")
    ...
    def run_karyogram(self):
        ...
        # also refresh the other two
        self.load_additional_karyograms()

        self.tabs.setCurrentIndex(1)
        QMessageBox.information(self, "Done", "Karyogram generated. Check the Karyogram tabs.")
