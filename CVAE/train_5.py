...
"""figure out which loss fn to use for the anomaly score reporting and train////"""
import torch
...
def calculate_class_thresholds(anomaly_scores, class_labels, output_dir):
  ...
  for cls in unique_classes:
    ...
    class_stats[cls] = {'mean': cls_mean, 'var': cls_var}
    class_thresholds[cls] = cls_threshold
    class_thresholds_normalized[cls] = (cls_threshold - global_min) / (global_max - global_min)
...
def train(batch, model, optimizer, device, ssim_fn, beta, gamma, alpha):
    ...
    optimizer.zero_grad()
    total_loss, recon_loss, kld_loss, ssim_loss = loss_function(x_recon, x, mu, logvar, ssim_fn, beta, gamma, alpha, bsize)
    total_loss.backward()
    grad_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
    print(f"Grad Norm: {grad_norm:.4f}")
    optimizer.step()
    return total_loss.item(), recon_loss.item(), kld_loss.item(), ssim_loss.item(), total_loss, y
...
def validate(batch, model, device, ssim_fn, beta, gamma, alpha):
    ...
    with torch.no_grad():
        x_recon, mu, logvar = model(x, y)
        total_loss, bce, kld, ssim_loss = loss_function(x_recon, x, mu, logvar, ssim_fn, beta, gamma, alpha,bsize)
        return total_loss.item(), bce.item(), kld.item(), ssim_loss.item()
