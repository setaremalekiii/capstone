...
def train(batch, model, optimizer, device, beta, gamma):
  model.train()
  x, y = batch
  ...
  optimizer.zero_grad()
  loss, recon_loss, kld_loss = loss_function(x_recon, x, mu, logvar, bsize, beta, gamma)
  loss.backward()
...
def validate(batch, model, device, beta, gamma):
  model.eval()
  x, y = batch
  x, y = x.to(device), y.to(device)
  bsize = x.size(0)
...
def plot_training_loss(total_bce_losses, total_kld_losses, total_val_loss, n_epochs):
  fig = plt.figure()
...
def train_model(model, optimizer, scheduler, train_dataloader, val_dataloader, device, n_epochs, output_dir, patience, min_beta_value):
  ...
  total_bce_losses, total_kld_losses = [], []
  total_val_bce_losses, total_val_kld_losses = [], []
  best_val_loss = float("inf")
  

  for epoch in range(n_epochs):
    beta = min(min_beta_value + 1.0 * epoch / n_epochs, 1.0)
    gamma = max(1.0 - 0.5 * epoch / n_epochs, 0.5)
    print(f"Epoch: {epoch+1}/{n_epochs} | Beta: {beta:.4f} | Gamma: {gamma:.4f}")
    train_epoch_losses, train_bce_losses, train_kld_losses = [], [], []
    val_bce_losses, val_kld_losses, val_epoch_losses= [], [], []
    
    for batch_idx, train_batch in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch+1}/{n_epochs} [Train]")):
      train_batch_loss, total_bce_loss, train_kld_loss = train(train_batch, model, optimizer, device, beta, gamma)
      ...
      print(f"Epoch {epoch+1} Batch {batch_idx} - Loss: {train_batch_loss:.4f}, BCE: {total_bce_loss:.4f}, gamma:{gamma}, KLD: {train_kld_loss:.4f}, beta:{beta}")
      
    scheduler.step()

    for val_batch in tqdm(val_dataloader, desc=f"Epoch {epoch+1}/{n_epochs} [Val]"):
      val_loss, val_bce_loss, val_kld_loss = validate(val_batch, model, device, beta, gamma)
  ...
  rk_loss_plot = plot_training_loss(total_bce_losses, total_kld_losses, total_val_loss, stopped_epoch)
  rk_loss_plot.savefig(os.path.join(output_dir, 'training_loss_curve.png'))
  plt.close(rk_loss_plot)
  
  val_fig = plot_training_loss(total_val_bce_losses, total_val_kld_losses, total_val_loss, stopped_epoch)
